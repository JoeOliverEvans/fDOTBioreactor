...
if isfield(cylinder, 'muax') && isfield(cylinder, 'musx')
    ...
    mesh.kappax(indices) = mesh.kappax(indices) + kappax;
end
if isfield(cylinder, 'muam') && isfield(cylinder, 'musm')
    kappam = 1./(3*(cylinder.muam+cylinder.musm));
    mesh.kappam(indices) = mesh.kappam(indices) + kappam;
end
if isfield(cylinder, 'muax')
    mesh.muax(indices) = mesh.muax(indices) + cylinder.muax;
end
if isfield(cylinder, 'musx')
    mesh.musx(indices) = mesh.musx(indices) + cylinder.musx;
end
if isfield(cylinder, 'ri')
    mesh.ri(indices) = mesh.ri(indices) + cylinder.ri;
     mesh.c(indices)= mesh.c(indices) + (3e11/cylinder.ri);
end
if isfield(cylinder, 'muam')
    mesh.muam(indices) = mesh.muam(indices) + cylinder.muam;
end
if isfield(cylinder, 'musm')
    mesh.musm(indices) = mesh.musm(indices) + cylinder.musm;
end
if isfield(cylinder, 'muaf')
    ...
    if ~isfield(cylinder, 'muax')
        ...
        mesh.muax(indices) = mesh.muax(indices) + (cylinder.muaf);
    end
    
    mesh.muaf(indices) = mesh.muaf(indices) + cylinder.muaf;
end
if isfield(cylinder, 'tau')
    mesh.tau(indices) = mesh.tau(indices) + cylinder.tau;
end
if isfield(cylinder, 'eta')
    mesh.eta(indices) = mesh.eta(indices) + cylinder.eta;
end
if (isfield(cylinder, 'region') ~= 0)
    mesh.region(indices) = mesh.region(indices) + cylinder.region;
end
