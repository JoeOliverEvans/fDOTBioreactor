...
% mesh = load_mesh_altered_properties('../../cylinder_dense/cylinder_dense');
...
for rep = 1:samples
    ...
    if channel_selection == true
        [noisy_img, data0, J] = recon_noisy_channel_selection(rep, mesh, solver ,opt, data, xgrid, ygrid, zgrid, data0, noisy_img, reg_strength);
        j1(:,:,:,rep) = J;
    end
end

if channel_selection == false
    [noisy_img, data0, J] = recon_noisy_reg(mesh, solver, opt, all_datafl_clean, all_datax_clean, samples, reg_strength);
end
