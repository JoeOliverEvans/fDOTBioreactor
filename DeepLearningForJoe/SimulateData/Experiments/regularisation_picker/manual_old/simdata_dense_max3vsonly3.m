...
for rep = 1:samples
    ...
    threshold_fl = 0.01*max(data.amplitudefl);
    threshold_xx = 0.01*max(data.amplitudex);
    idx = ~(data.amplitudefl<threshold_fl | data.amplitudex<threshold_xx);
    
    for p = 1:4
        p
        recon_grid = tikhonov(J.complexm(idx,:)./data0.amplitudex(idx), 1*10^(p-5), data.amplitudefl(idx)./data.amplitudex(idx));
        
        recon_grid = reshape(recon_grid, length(xgrid), length(ygrid), length(zgrid));
        
        noisy_img(:,:,:,rep+p-1) = recon_grid;
    end
