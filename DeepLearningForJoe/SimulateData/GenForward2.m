...
radius = sizevar.r;
for i = 0:7
    srcx(i+1) = radius * cos(2*pi*i/8);
    srcy(i+1) = radius * sin(2*pi*i/8);
end

for i = 1:2:15
    detx((i+1)/2) = radius * cos(2*pi*i/16);
    dety((i+1)/2) = radius * sin(2*pi*i/16);
end

tmp = ones(8, 1) * (-70:20:70);
src = [repmat([srcx', srcy'], 8, 1), tmp(:)];

tmp = ones(8, 1) * (-60:20:60);
det = [repmat([detx', dety'], 7, 1), tmp(:)];
link = [];
for i=1:length(src)
    for j=1:length(det)
        if norm(src(i,:)-det(j,:))>5 && norm(src(i,:)-det(j,:))<130
            link = [link; [i, j, 1]];
        end
    end
end
